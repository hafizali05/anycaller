// app-screens-home.jsx — marketing landing page for any/call
// Editorial, voice-forward. Reuses Wordmark, Waveform, StaticWaveform, Button,
// Icon, Tag, StatusPill, SectionLabel from app-components.jsx.

// ────────────────────────────────────────────────────────────────────
// Shared bits used only by the landing page
// ────────────────────────────────────────────────────────────────────

// Hairline horizontal rule with optional label
function HomeRule({ label, color }) {
...
function Display({ children, size = 88, color, max = 1000 }) {
  return (
    <h1 style={{
      margin: 0, fontFamily: 'var(--font-display)', fontStyle: 'italic',
      fontWeight: 400, fontSize: size, lineHeight: 1.02,
      letterSpacing: '-0.025em', color: color || 'var(--ink)',
      maxWidth: max, textWrap: 'pretty',
    }}>{children}</h1>
  );
}
...
// ────────────────────────────────────────────────────────────────────
// Hero — "What should we call about?"
// ────────────────────────────────────────────────────────────────────
function HomeHero() {
  return (
    <section style={{
      ...
      {/* Faint marginalia in the top right corner */}
      <div style={{
      ...
      {/* Left column — headline + composer */}
      <div>
        <Eyebrow color="var(--accent)" dot="live">An AI dialer for the rest of us</Eyebrow>
        ...
        <p style={{
          marginTop: 26, maxWidth: 540, fontSize: 18, lineHeight: 1.5,
          color: 'var(--ink-2)', textWrap: 'pretty',
        }}>
          Describe the conversation in plain language. We build the agent,
          dial the numbers you upload, and hand back transcripts, recordings,
          and the data you asked for — structured.
        </p>

        {/* Composer-style affordance */}
        <div style={{
          ...
          <p style={{
            ...
            "I'm selling accounting software to small businesses. Ask if
            they use anything today, what it is, and what they like or
            dislike about it…
            <span style={{
              display: 'inline-block', width: 7, height: 18, background: 'var(--accent)',
              verticalAlign: '-3px', marginLeft: 2, animation: 'ac-caret 0.9s steps(1) infinite',
            }} />"
          </p>
...
// A floating "live call in progress" card for the hero right column
function HeroLiveCard() {
  ...
  return (
    <div style={{ position: 'relative' }}>
      {/* Behind-card decoration: a stack edge to hint there are more calls */}
      <div style={{
        position: 'absolute', inset: 18, transform: 'rotate(-1.5deg)',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <Eyebrow color="var(--accent)" dot="live">Live · in progress</Eyebrow>
          ...
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 26, color: 'var(--accent)', letterSpacing: '-0.02em', lineHeight: 1 }}>{fmt(elapsed)}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-3)', marginTop: 4, letterSpacing: '0.1em', textTransform: 'uppercase' }}>elapsed</div>
          </div>
        </div>

        {/* Waveform */}
        <div style={{ marginTop: 18, background: 'var(--paper)', borderRadius: 12, padding: '14px 16px', boxShadow: 'inset 0 0 0 1px var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <Eyebrow color="var(--accent)" dot>Ava speaking</Eyebrow>
        ...
        <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <HeroTurn who="them" t="00:47" text="We mostly like it — but the payroll add-on got expensive after the last price" live />
        </div>

        {/* Live extraction strip */}
        <div style={{
          marginTop: 16, background: 'var(--paper)',
...
function HeroTurn({ who, t, text, live }) {
  const isAva = who === 'ava';
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '46px 1fr', gap: 10, alignItems: 'baseline' }}>
      ...
      <div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: isAva ? 'var(--accent)' : 'var(--ink-3)' }}>
          {isAva ? 'Ava' : 'Jin'}
        </div>
        <div style={{
          fontFamily: isAva ? 'var(--font-display)' : 'var(--font-ui)',
          fontStyle: isAva ? 'italic' : 'normal',
          fontSize: isAva ? 14.5 : 13.5, color: 'var(--ink)', lineHeight: 1.45, marginTop: 1,
        }}>
          {text}{live && <span style={{
            display: 'inline-block', width: 5, height: 12, background: 'var(--ink-4)',
...
function MiniField({ k, v, pct, pulsing }) {
  return (
    <div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--ink-3)' }}>{k}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginTop: 3 }}>
        <span style={{ fontSize: 12.5, color: 'var(--ink)', fontStyle: pulsing ? 'italic' : 'normal' }}>{v}</span>
        {pct != null && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--sage)' }}>{pct}%</span>}
        {pulsing && <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--accent)', animation: 'ac-blink 1.4s infinite' }} />}
      </div>
    </div>
  );
}
    ...
    <section style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 36 }}>
        ...
        <div style={{ maxWidth: 280, fontSize: 13.5, color: 'var(--ink-2)', lineHeight: 1.5, paddingBottom: 8 }}>
          Every field is editable before you launch. 40% of accounts ship the
          first draft as-is.
        </div>
      </div>

      <div style={{
        display: 'grid', gridTemplateColumns: '1fr 50px 1fr', gap: 0,
        alignItems: 'stretch',
      }}>
        {/* Brief */}
        <div style={{
          background: 'var(--paper)', borderRadius: 14,
          padding: '28px 30px', boxShadow: 'inset 0 0 0 1px var(--border)',
        }}>
          ...
          <p style={{
            ...
            "I'm selling accounting software to small businesses in the US.
            Please call the business, confirm you're speaking with someone who
            handles their books, and ask: <span style={{ background: 'var(--accent-soft)', padding: '0 3px' }}>whether they use any accounting
            software</span>, <span style={{ background: 'var(--accent-soft)', padding: '0 3px' }}>which one</span>, <span style={{ background: 'var(--accent-soft)', padding: '0 3px' }}>what they like and dislike</span>, and
            whether they'd be open to a 15-minute demo next week. Be friendly
            and don't push if they say no."
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Eyebrow color="var(--accent)" dot>Ava · friendly · Sage</Eyebrow>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-4)' }}>v3 · 1.2s</span>
          </div>

          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 6 }}>Opening line</div>
            <div style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 16.5, lineHeight: 1.45, color: 'var(--ink)' }}>
              "Hi, this is Ava calling from Numerus. I work with small
              businesses on their accounting tools — do you have a quick minute?"
            </div>
          </div>

          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>Objectives</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'baseline', fontSize: 13.5, color: 'var(--ink)' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-3)', minWidth: 16 }}>0{i + 1}</span>
                  <span>{s}</span>
                </div>
              ))}
            </div>
          ...
          <div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>Extraction schema</div>
            <div style={{
              ...
              <div>{'{'}</div>
              <div style={{ paddingLeft: 16 }}>contact_role: <span style={{ color: 'var(--slate)' }}>string</span>,</div>
              <div style={{ paddingLeft: 16 }}>current_software: <span style={{ color: 'var(--slate)' }}>string</span>,</div>
              <div style={{ paddingLeft: 16 }}>likes: <span style={{ color: 'var(--slate)' }}>text</span>,</div>
              <div style={{ paddingLeft: 16 }}>dislikes: <span style={{ color: 'var(--slate)' }}>text</span>,</div>
              <div style={{ paddingLeft: 16 }}>demo_interest: <span style={{ color: 'var(--accent)' }}>'yes' | 'no' | 'maybe'</span>,</div>
              <div style={{ paddingLeft: 16 }}>callback_time: <span style={{ color: 'var(--slate)' }}>datetime?</span></div>
              <div>{'}'}</div>
            </div>
          </div>
        </div>
...
// ────────────────────────────────────────────────────────────────────
// What you get back — three columns
// ────────────────────────────────────────────────────────────────────
function WhatYouGet() {
  return (
    <section style={{ padding: '96px 56px', background: 'var(--paper)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.05fr 0.9fr 1.05fr', gap: 22 }}>
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[
              ...
              { who: 'them', t: '00:44', text: "Every month I'm doing reconciliation by hand — two evenings of my life." },
            ].map((l, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '40px 1fr', gap: 8 }}>
                ...
                <div style={{
                  ...
                  fontSize: 12.5, color: l.who === 'ava' ? 'var(--accent-2)' : 'var(--ink)',
                  lineHeight: 1.45,
                }}>
                  {l.text}
                </div>
        ...
        >
          <div style={{
            background: 'var(--paper-2)', borderRadius: 10, padding: '22px 16px',
            display: 'flex', flexDirection: 'column', gap: 16,
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button style={{
        ...
        <ResultCard
          ...
          hint="CSV · JSON · webhook"
          title="The fields you asked for"
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {[
              ...
              <div key={k} style={{
                display: 'grid', gridTemplateColumns: '100px 1fr 36px', gap: 8,
                alignItems: 'baseline', padding: '7px 10px',
                background: 'var(--paper-2)', borderRadius: 6,
              }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-3)' }}>{k}</span>
                <span style={{ fontSize: 12.5, color: 'var(--ink)' }}>{v}</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--sage)', textAlign: 'right' }}>{c}%</span>
              </div>
            ))}
          </div>
        </ResultCard>
...
function ResultCard({ eyebrow, hint, title, children }) {
  return (
    <div style={{
      background: 'var(--paper)', borderRadius: 14,
      padding: '22px 22px', boxShadow: 'inset 0 0 0 1px var(--border)',
...
function UseCases() {
  return (
    <section style={{ padding: '96px 56px', background: 'var(--paper-2)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40 }}>
        ...
        <div style={{ maxWidth: 280, fontSize: 13.5, color: 'var(--ink-2)', lineHeight: 1.5, paddingBottom: 8 }}>
          A handful of patterns our customers run today. Bring your own — the
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18 }}>
        {USE_CASES.map((u, i) => (
          <div key={i} style={{
            background: 'var(--paper)', borderRadius: 14, padding: '22px 22px',
            boxShadow: 'inset 0 0 0 1px var(--border)',
            display: 'flex', flexDirection: 'column', gap: 16, minHeight: 360,
          }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Tag tone={u.tone}>{u.persona}</Tag>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-4)' }}>0{i + 1}</span>
            </div>
            <p style={{
              margin: 0, fontFamily: 'var(--font-display)', fontStyle: 'italic',
            ...
            <div style={{ flex: 1 }} />
            <div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 6 }}>Extracts</div>
    ...
    <section style={{ padding: '96px 56px', background: 'var(--paper)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
        <StepCard
          ...
          stat="~ 2 minutes"
        >
          <div style={{ background: 'var(--paper-2)', borderRadius: 10, padding: '14px 16px', boxShadow: 'inset 0 0 0 1px var(--border)' }}>
            <Eyebrow>Brief</Eyebrow>
            <p style={{ margin: '6px 0 0', fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 14, color: 'var(--ink)', lineHeight: 1.5 }}>
              "Ask 5 questions about how restaurants handle online reservations…"
            </p>
          </div>
        </StepCard>
        ...
        >
          <div style={{ background: 'var(--paper-2)', borderRadius: 10, padding: '12px 14px', boxShadow: 'inset 0 0 0 1px var(--border)' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontFamily: 'var(--font-mono)', fontSize: 11.5, color: 'var(--ink-2)' }}>
...
function StepCard({ n, title, copy, stat, children }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
        <div style={{
          fontFamily: 'var(--font-mono)', fontSize: 56, lineHeight: 1,
          color: 'var(--accent)', letterSpacing: '-0.04em',
        }}>{n}</div>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-3)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{stat}</span>
      </div>
      <div>
...
// ────────────────────────────────────────────────────────────────────
// Live ribbon — a "look in on calls happening right now" strip
// ────────────────────────────────────────────────────────────────────
function LiveRibbon() {
  return (
    <section style={{
      padding: '64px 56px', background: 'var(--ink)', color: 'var(--paper)',
      position: 'relative', overflow: 'hidden',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
        <div>
          <Eyebrow color="var(--accent)" dot="live">Right now, on the network</Eyebrow>
          <div style={{
            marginTop: 14, fontFamily: 'var(--font-display)', fontStyle: 'italic',
            fontSize: 44, lineHeight: 1.1, letterSpacing: '-0.02em', maxWidth: 760,
          }}>
            Across all customers — last 60 seconds.
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 44, color: 'var(--accent)', letterSpacing: '-0.02em', lineHeight: 1 }}>
            1,284
          </div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--ink-4)', marginTop: 4 }}>
            calls in progress
          </div>
        </div>
      </div>

      {/* Ribbon of mini call cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 14 }}>
        {[
          ...
          <div key={i} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: c.status === 'live' ? 'var(--accent)' : 'var(--amber)', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: c.status === 'live' ? 'var(--accent)' : 'var(--amber)', animation: 'ac-blink 1.4s infinite' }} />
            ...
            <div style={{ marginTop: 12 }}>
              <Waveform bars={28} height={20} state={c.status === 'live' ? 'speaking' : 'listening'} gap={2} barWidth={2} fill color={c.status === 'live' ? 'var(--accent)' : 'var(--ink-4)'} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        marginTop: 32, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 24,
        paddingTop: 24, borderTop: '1px solid rgba(255,245,230,0.1)',
      }}>
        {[
          ...
          <div key={i}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--ink-4)' }}>{k}</div>
            <div style={{ marginTop: 4, fontFamily: 'var(--font-mono)', fontSize: 18, color: 'var(--paper)' }}>{v}</div>
          </div>
        ))}
      </div>
    </section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '0.7fr 1.3fr', gap: 56, alignItems: 'start' }}>
        <div>
          ...
          <Display size={40} max={420}>
            Built to keep your brand off a regulator's desk.
          </Display>
        </div>
...
function PriceCard({ name, price, per, subtitle, features, cta, ctaVariant, featured }) {
  return (
    <div style={{
      ...
      boxShadow: featured
        ? '0 22px 60px -28px rgba(26,23,20,0.6)'
        : 'inset 0 0 0 1px var(--border)',
      display: 'flex', flexDirection: 'column', gap: 20, minHeight: 480,
      position: 'relative',
    }}>
      {featured && (
        <span style={{
      ...
      <div>
        <div style={{
          ...
          textTransform: 'uppercase', color: featured ? 'var(--ink-4)' : 'var(--ink-3)',
        }}>{name}</div>
        <div style={{ marginTop: 8, display: 'flex', alignItems: 'baseline', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: featured ? 'var(--ink-4)' : 'var(--ink-3)' }}>
            {per}
          </span>
        </div>
      ...
      <Button variant={ctaVariant} size="lg" full>
        {cta} <Icon name="arrow" size={13} color={ctaVariant === 'accent' || ctaVariant === 'primary' ? '#fff' : 'var(--ink)'} />
      </Button>
    </div>
  );
}
...
function FinalCTA() {
  return (
    <section style={{
      padding: '120px 56px 100px', background: 'var(--paper)',
      ...
      <p style={{
        margin: '28px auto 0', maxWidth: 540, fontSize: 16, color: 'var(--ink-2)', lineHeight: 1.55,
      }}>
        Ten minutes free. No card. Talk to your agent first if you want — we
      ...
      <div style={{
        marginTop: 60, display: 'flex', justifyContent: 'center', gap: 14,
        fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-3)',
        letterSpacing: '0.12em', textTransform: 'uppercase',
      }}>
...
function HomeFooter() {
  ...
  const link = { fontSize: 13, color: 'var(--ink-2)', cursor: 'pointer' };
  const head = {
    fontFamily: 'var(--font-mono)', fontSize: 10.5,
    letterSpacing: '0.18em', textTransform: 'uppercase',
  ...
  return (
    <footer style={{ padding: '48px 56px 36px', background: 'var(--paper-2)', borderTop: '1px solid var(--border)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr 1fr 1fr', gap: 32 }}>
        <div>
          <Wordmark size={20} live />
          <p style={{ margin: '14px 0 0', fontSize: 13, color: 'var(--ink-2)', maxWidth: 280, lineHeight: 1.5 }}>
            A voice-forward tool for anyone who needs a few hundred
            conversations completed — well.
          </p>
        </div>
        <div style={col}>
...
Object.assign(window, {
  HomeScreen, HomeNav, HomeHero, TransformationSection,
  WhatYouGet, UseCases, HowItWorks, LiveRibbon, TrustStrip,
  Pricing, FinalCTA, HomeFooter,
});
