// app-components.jsx — shared primitives for the any/call prototype
// Exports to window so other Babel scripts can use them.

// ─────────────────────────────────────────────────────────────
// Wordmark — any/call with a live record dot
// ─────────────────────────────────────────────────────────────
function Wordmark({ size = 18, live = false, color }) {
  ...
  return (
    <span style={{
      ...
      <span style={{ position: 'relative', display: 'inline-block', width: size * 0.5, height: size * 0.5 }}>
        ...
        {live && <span style={{
          position: 'absolute', inset: 0, borderRadius: '50%',
          background: 'var(--accent)', animation: 'ac-pulse-ring 1.4s ease-out infinite',
        }} />}
      </span>
      <span>any<span style={{ color: 'var(--ink-3)', fontWeight: 400, margin: '0 0.05em' }}>/</span>call</span>
    </span>
  );
}
...
// ─────────────────────────────────────────────────────────────
function Waveform({ bars = 28, height = 36, state = 'speaking', color, gap = 3, barWidth = 3, fill = false }) {
  // Stable per-bar seed so re-renders don't reshuffle heights
  const seed = React.useMemo(() => {
    const arr = [];
    for (let i = 0; i < bars; i++) {
      // shape: center-loud, edge-quiet
  ...
  return (
    <div style={{
      ...
      height, width: fill ? '100%' : 'auto', justifyContent: fill ? 'space-between' : 'flex-start',
    }}>
      {seed.map((s, i) => (
        <div key={i} style={{
          ...
          background: c, borderRadius: barWidth,
...
// Static waveform — for transcript bubbles, completed recordings.
// Renders as deterministic SVG path.
function StaticWaveform({ width = 240, height = 28, color, seed = 7, progress = 1, bars }) {
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ display: 'block' }}>
      {cells.map((v, i) => {
        ...
        const x = i * w + w / 2;
        return (
          <line key={i} x1={x} x2={x} y1={(height - h) / 2} y2={(height + h) / 2}
            stroke={i <= playedTo ? c : 'var(--ink-4)'} strokeWidth={Math.max(1.5, w * 0.55)}
            strokeLinecap="round" />
        );
      })}
    </svg>
...
function StatusPill({ status, size = 'sm' }) {
  ...
  return (
    <span style={{
      ...
      padding: pad, borderRadius: 999, background: s.bg,
...
// Buttons
// ─────────────────────────────────────────────────────────────
function Button({ variant = 'primary', size = 'md', children, onClick, disabled, icon, style, full }) {
  const base = {
  ...
  return (
    <button onClick={disabled ? undefined : onClick} disabled={disabled} style={{
      ...base, ...sizes[size], ...variants[variant], ...style,
    }}>
      {icon}{children}
    </button>
  );
...
// ─────────────────────────────────────────────────────────────
// Icon — minimal hairline glyphs (1.5px stroke)
// ─────────────────────────────────────────────────────────────
function Icon({ name, size = 16, color = 'currentColor', strokeWidth = 1.5 }) {
  const p = { width: size, height: size, viewBox: '0 0 24 24', fill: 'none',
    stroke: color, strokeWidth, strokeLinecap: 'round', strokeLinejoin: 'round' };
  switch (name) {
    case 'phone':   return <svg {...p}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.79 19.79 0 0 1 2.12 4.18 2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92Z"/></svg>;
    case 'brief':   return <svg {...p}><path d="M4 4h12l4 4v12H4z"/><path d="M16 4v4h4"/><path d="M7 10h6M7 13h10M7 16h8"/></svg>;
    ...
    case 'dial':    return <svg {...p}><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>;
    case 'feed':    return <svg {...p}><path d="M3 12h4l3-8 4 16 3-8h4"/></svg>;
    ...
    case 'sparkle': return <svg {...p}><path d="M12 3v6m0 6v6M3 12h6m6 0h6M5.5 5.5l4 4m5 5l4 4M18.5 5.5l-4 4m-5 5l-4 4"/></svg>;
    case 'upload':  return <svg {...p}><path d="M12 3v13M5 10l7-7 7 7M3 21h18"/></svg>;
    ...
    case 'voicemail': return <svg {...p}><circle cx="6" cy="14" r="4"/><circle cx="18" cy="14" r="4"/><path d="M6 14h12"/></svg>;
    case 'clock':   return <svg {...p}><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>;
    case 'arrowU':  return <svg {...p}><path d="M12 19V5M5 12l7-7 7 7"/></svg>;
    case 'arrowD':  return <svg {...p}><path d="M12 5v14M5 12l7 7 7-7"/></svg>;
    case 'chev':    return <svg {...p}><path d="M9 6l6 6-6 6"/></svg>;
    case 'wave':    return <svg {...p}><path d="M2 12h2l2-6 4 12 4-16 4 12 2-6h4"/></svg>;
    ...
    case 'square':  return <svg {...p}><rect x="5" y="5" width="14" height="14" rx="1.5"/></svg>;
    default: return null;
  }
}
...
// Tag / chip
// ─────────────────────────────────────────────────────────────
function Tag({ children, tone = 'neutral', mono = true }) {
  const tones = {
  ...
  return (
    <span style={{
      ...
      background: t.bg, color: t.color, boxShadow: t.box,
...
// ─────────────────────────────────────────────────────────────
// Streaming text — types out a string char-by-char
// ─────────────────────────────────────────────────────────────
function StreamText({ text, speed = 14, delay = 0, caret = true, onDone, trigger = 0 }) {
  const [n, setN] = React.useState(0);
  const [started, setStarted] = React.useState(delay === 0);
  React.useEffect(() => { setN(0); setStarted(delay === 0); }, [text, trigger]);
  React.useEffect(() => {
  ...
  return (
    <span>{text.slice(0, n)}{caret && !done && started && (
      <span style={{
        display: 'inline-block', width: '0.55em', height: '1em',
        background: 'var(--accent)', verticalAlign: '-0.13em', marginLeft: 1,
...
// ─────────────────────────────────────────────────────────────
// Section heading — small caps mono label above content
// ─────────────────────────────────────────────────────────────
...
Object.assign(window, {
  Wordmark, Waveform, StaticWaveform,
  StatusPill, STATUS,
  Button, Icon, Tag,
  StreamText, SectionLabel,
});
